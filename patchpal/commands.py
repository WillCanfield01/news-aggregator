...
def _resolve_channel_id(text: str, body: dict, client, logger):
    t = (text or "").strip()
    if t.lower().endswith("here"):  # `/patchpal set-channel here`
    ...
    if not m:
        return None
    # Best effort name→ID (requires channels:read,groups:read if used)
            ...
            with SessionLocal() as db:
                ...
                if text.startswith("set-channel"):
                    ...
                    if not cid:
                        respond(
                            ...
                            "_Tip: In your Slack app, enable “Escape channels, users, and links sent to your app”.*"
                        ); return
                ...
                if text.startswith("post-now"):
                    if not ws.post_channel:
                        respond("Set a channel first: `*/patchpal set-channel here*`.")
                        return
                    ...
                    client.chat_postMessage(
                        ...
                        blocks=as_slack_blocks(items, tone=ws.tone or "simple"),
                    )
